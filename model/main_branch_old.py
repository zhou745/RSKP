...
class WSTAL(nn.Module):
    ...
    def forward(self, x,pseudolabel=None,use_drop = True, pertube_factor = -0.1,
                pertube_offset=1.0,pseudolabel_rate=0.2):
        n, t, _ = x.size()

        # feature embedding
        x = self.feature_embedding(x)
        # if pertube_factor>0.:
        #     noise = (torch.rand(self.feature_embedding[0].weight.shape)*2-1) * pertube_factor
        #     noise = noise.to(x.device)
        #     noise = noise*self.feature_embedding[0].weight.std()
        #     delta_x = x@noise.t()
        #     x = x + delta_x
        ...
        # o_vid_ca_pred_old, o_vid_cw_pred_old, o_att_old, o_frm_pred_old = self.PredictionModule_old(x)
