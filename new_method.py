...
class Processor():
    def __init__(self, args):
        ...
        # weight = torch.load("./centers_wk.pkl")
        # weight = weight.to(self.device)

        # weight_ori = self.model.ac_center.data.clone()
        # weight_fuse = weight_ori*0.+weight*1.0
        # self.model.ac_center.data.copy_(weight_fuse)
        ...
        #set the predictor parameters
        # ac_centers = self.model.ac_center.data.clone()
        # self.predictor.set_SVD(ac_centers)
        # Optimizer Setting
        # if True:
        #     parameters = [{"params": self.model.parameters()}, {"params": self.predictor.parameters()}]
        # else:
    ...
    def train(self):
        ...
        for idx in self.args.load_dist_ckpt:
            ...
            for num, sample in tqdm(enumerate(self.train_data_loader)):
                ...
                pred_label = utils.utils2.fuse_current_pred(d_o_out, d_m_out, d_em_out, f_labels, self.args)
                # pred_label = utils.utils.smooth_pred(pred_label).detach().cpu()
        ...
        for epoch in epoch_range:
            ...
            if epoch in self.args.dist_epoch:
                ...
                self.args.dist_temperature = self.args.dist_temperature_decay[idx]
            ...
            for num, sample in enumerate(self.train_data_loader):
                ...
                if epoch<=100:
                    ...
                    self.args.spl_loss_weight = 1.0
                    # attn_label = feature_list[-1][idx[0]].to(self.device)-feature_list[-2][idx[0]].to(self.device)
                    pseudolabel_rate = 0.1
                    attn_label = None
                    noise_rate = None
                elif epoch <=300:
                    ...
                    # attn_label = (feature_list[-1][idx[0]].to(self.device) - feature_list[-2][idx[0]].to(self.device))*2.
                    self.args.spl_loss_weight = 1.0
                    pseudolabel_rate = 0.1
                    attn_label = None
                    noise_rate = None
                    ...
                    # pred_label = feature_list[-1][idx[0]].to(self.device)
                    pred_label = feature_list[-1][idx[0]].to(self.device)*2.25-feature_list[-2][idx[0]].to(self.device)*2.25
                    # attn_label = (feature_list[-1][idx[0]].to(self.device) - feature_list[-2][idx[0]].to(self.device))*2.
                    self.args.spl_loss_weight = 1.0
                    pseudolabel_rate = 0.1
                    attn_label = None
                ...
                #sharp predictions
                # fused_feature = 0.85*em_out[0]+0.15*em_out[-1]
                # pred_svd = self.predictor(fused_feature)

                loss_dist = utils.utils2.compute_distloss(o_out, m_out, em_out, pred_label,self.args)
                ...
                #compute the predictor loss
                # loss_predictor = utils.utils2.compute_distloss(o_out, m_out, em_out, pred_svd,self.args)
