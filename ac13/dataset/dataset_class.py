...
class ACMDataset(Dataset):
    ...
    def __init__(self, args, phase="train", sample="random"):
        ...
        if self.phase == "train":
            ...
            self.data_list = list(open(os.path.join(self.data_dir, "split_train_13.txt")))
            self.data_list = [item.strip() for item in self.data_list]
        else:
            self.feature_dir = os.path.join(self.data_dir, "test")
            self.data_list = list(open(os.path.join(self.data_dir, "split_test_13.txt")))
            self.data_list = [item.strip() for item in self.data_list]
    ...
    def __getitem__(self, idx):
        ...
        if self.sample == "random":
            ...
            con_vid_spd_feature = con_vid_feature
        else:
        ...
        if self.phase == "train":
            return idx, con_vid_spd_feature, vid_label_t, vid_name
        else:
