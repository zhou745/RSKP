    ...
    for c_idx in range(c_len):
        ...
        if c_temp_seg_list.any():
            ...
            for j in range(len(grouped_c_temp_list)):
                ...
                c_scores = np.mean(int_temp_scores[grouped_c_temp_list[j], c_idx]) - (
                            c_left_outer + c_right_outer) / 2 + 0.3 * c_pred_scores[c_idx]
...
def get_proposal_oic(tempseg_list, int_temp_scores, c_pred, c_pred_scores, t_factor, lamb=0.25,
